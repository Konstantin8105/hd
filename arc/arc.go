...
func Assembly_Matrix() {
	...
	//         end
	//     end
	// endfunction global_vector=Assembly_Vector(global_vector,local_vector,LM,e)
	// %%% Assemble F_global
	// nen=size(LM,2);
	// for aa=1:nen
	//     mm=LM(e,aa);
	...
	//     end
	// endfunction [Klocal, Flocal]=GeomExactBeam_2D(elmDat, IEN, e, XX, soln, bf)
...
func solve_arclength() {
	...
	//     du = dutemp(1:end-1);
	//     dl = dutemp(end);
	// endfunctionfunction  [converged, du, dl, du1] = solve_arclength_split(timeStep, neq, iter, Kglobal, Rglobal, dof_force, Fext, assy4r, Du, Dl, ds, du1)
	//
	//     psi = 1.0;
	//
	...
	//
	//     du = -du2 + dl*du1;
	// endfunction td = timeSteppingParameters_Solid(tis, rho, dt)
	//
	// td = zeros(100,1);
	//
	// switch tis
